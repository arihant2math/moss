...
use crate::{error::{KernelError, Result}, fs::{
...
use ext4_view::{AsyncIterator, AsyncSkip, Ext4, Ext4Read, File, FollowSymlinks, Metadata, ReadDir, get_dir_entry_inode_by_name, Ext4Write};
use log::error;
...
#[async_trait]
impl<CPU> Inode for Ext4Inode<CPU> where CPU: CpuOps + Send + Sync {
    fn id(&self) -> InodeId {
...
impl<CPU> Ext4Filesystem<CPU> where CPU: CpuOps + Send + Sync {
    /// Construct a new EXT4 filesystem instance.
    pub async fn new(dev: BlockBuffer, id: u64) -> Result<Arc<Self>> {
        let dev_arc = Arc::new(dev);
        let inner = Ext4::load_with_writer(Box::new(dev_arc.clone()), Some(Box::new(dev_arc.clone()))).await?;
...
#[async_trait]
impl<CPU> Filesystem for Ext4Filesystem<CPU> where CPU: CpuOps + Send + Sync {
    fn id(&self) -> u64 {
        self.id
    }
