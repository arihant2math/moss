...
pub async fn sys_mount(
    ...
    let dev_name = UserCStr::from_ptr(dev_name).copy_from_user(&mut buf).await?;
    buf = [0u8; 1024];
    let dir_name = UserCStr::from_ptr(dir_name).copy_from_user(&mut buf).await?;
    let mount_point = VFS.resolve_path(Path::new(dir_name), VFS.root_inode(), &current_task_shared()).await?;
    buf = [0u8; 1024];
